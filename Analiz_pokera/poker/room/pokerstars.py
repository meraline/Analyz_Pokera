    ...
    def _parse_actions(self, actionlines):
        ...
        for line in actionlines:
            if line.startswith("Uncalled bet"):
                ...
                continue
            elif ' is disconnected' in line:  # skip is disconnected lines
                continue    
            elif ' has returned' in line:  # skip has returned lines
                continue    
            elif ' is sitting out' in line:  # skip is sitting out lines
                continue
            elif ' is connected' in line:  # skip is connected lines
                continue    
            elif ' has timed out' in line:  # skip has timed out lines
                continue
            elif '*"' in line:  # skip *" lines
                continue        
            elif ":" in line:
...
class PokerStarsHandHistory(hh._SplittableHandHistoryMixin, hh._BaseHandHistory):
    ...
    _winner_re = re.compile(r"Seat (\d+): (.+?) collected ((\d+)|\((\d+)\)) (?!from pot)")
